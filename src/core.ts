 ...
 *
 * Creates a Mapbox video source with a placeholder MP4, waits for Mapbox to
 ...
 *
 * @param map A Mapbox GL map instance. The map style may be loaded or still loading.
 ...
 *     onLoad: (video) => console.log('Stream playing', video.duration),
...
export function addHlsLayer(
   ...
   * that Mapbox creates asynchronously. Stops polling once found or destroyed.
   */
  function init() {
    if (destroyed) return;
    addSourceAndLayer();
  ...
  return {
    ...
    /**
     * Returns the HTML video element created by Mapbox, or null if not yet initialized.
     */
    getVideo() {
      return videoEl;
    },
